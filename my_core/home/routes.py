from flask import render_template, flash, \
                    ...
                    g, current_app, jsonify, Blueprint
...
from my_core.utils.translate import translate
from my_core.models import User, Post, Message, Notification
...
def index():
    ...
    if postform.validate_on_submit():
        ...
        post = Post(body=postform.post.data, author=current_user, language=language)
        db.session.add(post)
        db.session.commit()
    ...
    return render_template('home/index.html', title=_('Home'), user=user,
                           posts=posts.items, form=postform,
...
def user(username):
    ...
    next_url = url_for('home.user', username=user.username, 
                    page=posts.next_num) if posts.has_next else None
    prev_url = url_for('home.user', username=user.username, 
            page=posts.prev_num) if posts.has_prev else None
    return render_template('home/user.html', user=user, posts=posts.items,
                           next_url=next_url, prev_url=prev_url)
...
def edit_profile():
    ...
    return render_template('home/edit_profile.html', title=_('Edit Profile'),
                           form=edit_profile_form)


@home_bp.route('/follow/<username>')
@login_required
def follow(username):
...
@home_bp.route('/unfollow/<username>')
@login_required
...
def explore():
    ...
    return render_template('home/index.html', title=_('Explore'),posts=posts.items,
                           next_url=next_url, prev_url=prev_url)
...
def search():
    ...
    prev_url = url_for('home.search', q=g.search_form.q.data, page=page - 1) \
        if page > 1 else None
    return render_template('home/search.html', title=_('Search'), posts=posts,
                           next_url=next_url, prev_url=prev_url)


@home_bp.route('/user/<username>/popup')
@login_required
def user_popup(username):
    user = User.query.filter_by(username=username).first_or_404()
    return render_template('includes/user_popup.html', user=user)
...
def send_message(recipient):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('home.user', username=recipient))
...
def messages():
    ...
    messages = current_user.messages_received.order_by(
        Message.timestamp.desc()).paginate(
            page, current_app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('home.messages', page=messages.next_num) \
        if messages.has_next else None
    prev_url = url_for('home.messages', page=messages.prev_num) \
        if messages.has_prev else None
    return render_template('home/messages.html', messages=messages.items,
                           next_url=next_url, prev_url=prev_url)
